...
def create_one_restaurant():
    ...
    try:
        ...
        return (
            jsonify(
                {"details": "Invalid restaurant data. Missing required restaurant keys"}
            ),
            400,
        )
...
def update_one_restaurant(restaurant_id):
    ...
    try:
        ...
        return jsonify({"msg": "Missing needed restaurant data"}), 400
...
def delete_one_restaurant(restaurant_id):
    ...
    return (
        jsonify(
            {
                "details": f"Restaurant {restaurant_to_delete.restaurant_id} {restaurant_to_delete.name} successfully deleted"
            }
        ),
        200,
    )
...
# HISTORICAL SITES
historicalsites_bp = Blueprint(
    "historicalsites_bp", __name__, url_prefix="/historicalsites"
)
...
def create_one_historicalsites():
    ...
    if not all(key in request_data for key in required_keys):
        return (
            jsonify({"details": "Invalid site data. Missing required site keys"}),
            400,
        )
    ...
    try:
        ...
        return jsonify({"details": "Invalid data. Invalid site values provided"}), 400
...
def update_one_historicalsites(sites_id):
    ...
    try:
        ...
        return jsonify({"details": "Missing needed site data"}), 400
...
def delete_one_historicalsites(sites_id):
    ...
    return (
        jsonify(
            {
                "details": f"Historical site {historicalsites_to_delete.sites_id} {historicalsites_to_delete.name} successfully deleted"
            }
        ),
        200,
    )
...
def create_one_service():
    ...
    try:
        ...
        return (
            jsonify(
                {"details": "Invalid service data. Missing required service keys."}
            ),
            400,
        )
...
def update_one_service(service_id):
    ...
    try:
        ...
        return jsonify({"details": "Missing needed service data"}), 400
...
def delete_one_service(service_id):
    ...
    return (
        jsonify(
            {
                "details": f"Black owned Service {service_to_delete.service_id} {service_to_delete.name} successfully deleted"
            }
        ),
        200,
    )
...
def create_one_store():
    ...
    try:
        ...
        return (
            jsonify({"details": "Invalid store data. Missing required store keys."}),
            400,
        )
...
def update_one_stores(store_id):
    ...
    try:
        ...
        return jsonify({"details": "Missing needed store data"}), 400
...
def delete_one_restaurant(store_id):
    ...
    return (
        jsonify(
            {
                "details": f"Store {store_to_delete.store_id} {store_to_delete.name} successfully deleted"
            }
        ),
        200,
    )
...
def get_model_from_id(cls, model_id):
    try:
        ...
        return abort(make_response({"details": f"invalid data: {model_id}"}, 400))

    chosen_model = cls.query.get(model_id)

    if chosen_model is None:
        return abort(
            make_response({"details": f"Could not find item with id: {model_id}"}, 404)
        )
